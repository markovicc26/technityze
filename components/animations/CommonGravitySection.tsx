...
export default function CommonGravitySection({
  ...
  useLayoutEffect(() => {
    ...
    if (animateOnScroll) {
      /* Re-trigger the drop every time the section re-enters the viewport
         (scrolling down from above OR scrolling back up from below). Each
         entry: tear down old physics + reset card DOM positions so Matter
         can measure fresh rects + run initPhysics() again. Each leave:
         cleanup + reset so the next entry starts clean. */
      const reset = () => {
        cleanupRef.current?.();
        cleanupRef.current = null;
        initializedRef.current = false;
        objectRefs.current.forEach((el) => {
          if (!el) return;
          el.style.left = "";
          el.style.top = "";
          el.style.transform = "";
